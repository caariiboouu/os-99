...
Item {
  ...
  PanelWindow {
    ...
    BorderSurface {
      ...
      // Pair the panel with the text it draws. Upstream reads Color.background
      // (colors.toml) while the labels below read Color.popups.text -- two
      // independent sources that only line up by luck, and do not line up at all
      // in a theme whose content is dark but whose chrome is light.
      //
      // This cannot be fixed in theme data. In OS 99 Noir, popups.text must be
      // LIGHT for this toast (drawn on the #2B2B2B colors.toml background) and
      // DARK for Ui/Dropdown and the bar's tray menus (drawn on
      // popups.background, with the bar's own black text). One value, two
      // contradictory requirements. Reading popups.background here removes the
      // contradiction: every popup surface then agrees, at 15.46:1.
